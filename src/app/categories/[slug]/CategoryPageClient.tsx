...
export default function CategoryPageClient({ slug }: { slug: string }) {
  ...
  return (
    <main className="min-h-screen bg-gray-50 dark:bg-surface-dark">
      <Navbar />
      <section className="pt-24 pb-8 px-4 sm:px-6 lg:px-8" style={{ background: `linear-gradient(135deg, ${category.color}22, ${category.color}08)` }}>
        <div className="max-w-7xl mx-auto">
          <Link href="/categories" className="inline-flex items-center gap-1 text-sm text-gray-600 hover:text-primary mb-4"><ArrowLeft size={14} /> All Categories</Link>
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-3xl" style={{ backgroundColor: `${category.color}20` }}>{category.icon}</div>
            <div>
          ...
          <div className="flex-1 overflow-x-auto scrollbar-hide">
            <div className="flex gap-2 min-w-min">
              {subcategories.map((sub) => (
                <button key={sub} onClick={() => setActiveSubcat(sub)} className={`px-4 py-2 rounded-pill text-sm whitespace-nowrap transition-all ${activeSubcat === sub ? 'bg-primary text-white shadow-md' : 'bg-gray-100 dark:bg-card-dark text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'}`}>{sub}</button>
              ))}
            </div>
          </div>
          <div className="relative flex-shrink-0">
            <button onClick={() => setSortOpen(!sortOpen)} className="flex items-center gap-1 px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 text-sm hover:bg-gray-50 dark:hover:bg-white/5">
              <SlidersHorizontal size={14} /><span className="hidden sm:inline">Sort</span><ChevronDown size={12} />
            </button>
            {sortOpen && (
              <div className="absolute right-0 top-full mt-1 w-40 bg-white dark:bg-card-dark rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-1 z-50">
                {['Newest', 'Price: Low-High', 'Price: High-Low', 'Rating', 'Popular'].map((opt) => (
                  <button key={opt} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-white/5" onClick={() => setSortOpen(false)}>{opt}</button>
                ))}
              </div>
            )}
          </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4">
            {allProducts.map((product, i) => (
              <motion.div key={product.id} className="group rounded-2xl bg-white dark:bg-card-dark shadow-soft overflow-hidden" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03, duration: 0.3 }} whileHover={{ y: -4 }}>
                <Link href={`/products/${product.id}`}>
                  <div className="relative h-32 sm:h-36 bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
                    ...
                    <button className="absolute top-2 right-2 w-7 h-7 rounded-full bg-white/90 dark:bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity" aria-label="Add to wishlist" onClick={(e) => e.preventDefault()}><Heart size={12} className="text-gray-600" /></button>
                  </div>
                </Link>
                <div className="p-3">
                  <Link href={`/products/${product.id}`}><h3 className="font-medium text-sm truncate hover:text-primary transition-colors">{product.name}</h3></Link>
                  <p className="text-xs text-gray-500 mt-0.5">{product.vendor}</p>
                  <div className="flex items-center gap-1 mt-1"><Star size={10} className="text-warning fill-warning" /><span className="text-[10px] text-gray-500">{product.rating}</span></div>
                  <div className="flex items-center justify-between mt-2">
                    <span className="font-heading font-bold text-primary text-sm">₦{product.price.toLocaleString()}</span>
                    <button className="w-7 h-7 rounded-full bg-primary/10 flex items-center justify-center hover:bg-primary hover:text-white transition-colors" aria-label="Add to cart" onClick={(e) => e.preventDefault()}><ShoppingCart size={12} /></button>
                  </div>
                </div>
              </motion.div>
            ))}
          </div>
          <div className="text-center mt-10"><button className="btn-outline inline-flex items-center gap-2 text-sm">Load More</button></div>
        </div>
      </section>
      <Footer />
    </main>
